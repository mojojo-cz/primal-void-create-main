...
-- 修正额外学员数统计
-- 创建时间：2025-01-22
-- 功能：升级get_schedule_plans_with_stats函数，分别统计总学员和额外学员
-- =============================================================================

-- 升级 get_schedule_plans_with_stats 函数，修正额外学员数统计逻辑
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(total_participant_stats.total_participants, 0)::INTEGER AS plan_participants_count,
    COALESCE(extra_participant_stats.extra_participants, 0)::INTEGER AS extra_participants_count,
  ...
  ) total_participant_stats ON true
  LEFT JOIN LATERAL (
    SELECT COUNT(*) as extra_participants
    FROM public.plan_participants pp
    WHERE pp.plan_id = sp.id AND pp.status = 'active'
  ) extra_participant_stats ON true
...
-- 更新函数注释
COMMENT ON FUNCTION get_schedule_plans_with_stats IS '获取课表计划列表，正确统计总学员和额外学员数。';
